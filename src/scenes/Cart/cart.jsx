...
export const Cart = () => {
  ...
  useEffect(() => {
    const fetchCartData = async () => {
      try {
        const response = await fetch(`http://localhost:3001/user/cart/${id}`, {
          headers: {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/user/order/${id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const removeAddress = async (removeAddressId) => {
    try {
      const response = await fetch(
        `http://localhost:3001/user/address/remove/${id}`,
        {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div className="w-[100vw] min-h-[100vh] bg-gray-100">
      ...
      {fetchCart && fetchCart.length > 0 && (
        <div
          className={`fixed top-0 flex justify-center items-center w-[100vw] h-[100vh] ${
